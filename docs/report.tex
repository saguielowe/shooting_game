...
在进入技术分析之前，本节完整介绍游戏的玩法、模式和系统，为后续各技术模块提供上下文。建议评阅者先阅读本节，配合提交附件中的演示视频理解游戏的实际面貌。
...
武器可以通过拾取掉落物补充弹药或替换为更高优先级的武器（AI 会自行评估武器强度决定是否拾取）。
...
注意：\texttt{getAttackDamage()} 不知道"伤害+20\%"是哪来的——它只看到 \texttt{damageBonusMultiplier} 字段当前值是 1.25（吃了一个词条），它无法也不需区分这是哪个词条类型贡献的。这就是数据驱动的本质：\textbf{战斗逻辑对数据来源保持无知}。
...
% ============================================================
\section*{附录：编译与运行环境}
\addcontentsline{toc}{section}{附录：编译与运行环境}
