...
在进入技术分析之前，本节完整介绍游戏的玩法、模式和系统，为后续各技术模块提供背景信息。

\subsection{游戏介绍}

本项目是一个基于 Qt 图形界面的横版格斗对战游戏。两名角色在 2D 平台上进行对战——通过键盘操控角色移动、跳跃、近战/远程攻击、切换武器和释放法术，目标是将对手生命值削减至零。

游戏支持三种对局模式：

\begin{itemize}
  \item \textbf{人机对战（AI）}：玩家操控 P1，P2 由 AI 控制。适合单人快速体验。
  \item \textbf{双人对战（PVP）}：两名玩家使用同一键盘的不同键位分别操控 P1 和 P2。P1 键位为 WASD（移动）+ F（攻击）+ G/C（切换武器/法术），P2 键位为 IJKL（移动）+ O（攻击）+ N/U（切换武器/法术）。由于两人共用同一键盘，程序在设置界面检测按键冲突并在对局中过滤矛盾输入。
  \item \textbf{无尽模式（ENDLESS）}：玩家操控 P1 连续对抗 AI 对手，每轮结束后从三个随机词条中选择一个增益，AI 伤害逐轮递增。玩家阵亡即游戏终止，显示结算面板（总伤害、击杀数、生存时间）。无尽模式中 AI 拥有 \texttt{endlessImmortal} 属性——阵亡后有 1 秒保护期才判定真正死亡。
\end{itemize}

\subsection{开局设置}

启动程序后进入主菜单，点击“开始”进入设置页面。设置页面提供以下选项：

\begin{itemize}
  \item \textbf{游戏模式}：人机 / 双人 / 无尽，三种模式互斥选择。
  \item \textbf{总局数（BO）}：支持 Best of 3 / 5 / 7 / 自定义。BO3 意为率先获得 2 胜者赢得整场对局，每小局结束返回设置页复用原配置。
  \item \textbf{场景选择}：默认场景（常规平台布局）或草地场景（草丛可隐藏角色——不移动时自动蹲下进入隐匿状态）。
  \item \textbf{数值参数}：血量上限（默认 200）、实心球初始数量（默认 3）、步枪弹药（默认 5）、狙击弹药（默认 2）。可通过预设档位或手动输入调整。
  \item \textbf{法术选择}：P1 和 P2 各选一个法术——定身、隐身（仅人机可用）、铜头铁臂，或选“无法术”。双人模式中隐身不可选。选完后系统根据法术类型自动筛选对应该法术的词条池。
  \item \textbf{AI 法术策略}（仅人机/无尽）：CONFIG\_PICK（使用 P2 指定法术）、COUNTER\_PICK（根据玩家法术加权对策抽样）、RANDOM\_PICK（随机）。
\end{itemize}

点击“开始对局”后进入游戏主界面，60FPS 的帧循环即刻启动。画面上方显示双方状态栏——血条、武器图标、词条摘要、法术冷却进度——左上角为 P1，右上角为 P2。

\subsection{武器系统}

每位角色初始拥有拳击（无弹药限制），可通过拾取地面掉落物获取新武器。所有武器存放在\textbf{武器槽}中（默认 1 槽，可通过“百宝囊”词条扩展），按 C/N 键循环切换。五种武器的基础数据如下：

\begin{table}[H]
\centering
\begin{tabular}{p{2cm}p{1.8cm}p{1.8cm}p{2cm}p{2.8cm}}
\toprule
\textbf{武器} & \textbf{类型} & \textbf{伤害倍率} & \textbf{弹药限制} & \textbf{弹道} \\
\midrule
拳击 (punch) & 近战 & 5 & 无限 & 角色前方 40px 碰撞框 \\
小刀 (knife) & 近战 & 15 & 无限 & 角色前方 40px 碰撞框 \\
实心球 (ball) & 远程 & 1 & 初始 3 枚 & 抛物线，受重力 \\
步枪 (rifle) & 远程 & 30 & 初始 5 发 & 水平直线 \\
狙击枪 (sniper) & 远程 & 60 & 初始 2 发 & 水平直线 \\
\bottomrule
\end{tabular}
\caption{五种武器的参数对比。远程倍率指枪械子弹的伤害系数（狙击枪单发 60，步枪单发 30），实心球伤害则由动能公式另行计算（见下文伤害公式）。}
\end{table}

武器可以通过拾取掉落物补充弹药或替换为更高优先级的武器（AI 会自行评估武器强度决定是否拾取，若武器槽有空位则不会替换而是新增）。

\subsection{法术系统}

每名角色在开局时选定一个法术（或“无法术”），对局中按 Q（P1）/ U（P2）键释放。每种法术有独立的冷却时间，冷却期间按键无效，进度在状态栏以圆环形式显示。游戏包含三种法术：

\subsubsection{定身术（FREEZE）}

\begin{itemize}
  \item \textbf{释放条件}：自身不在受击硬直中。
  \item \textbf{效果}：对手 5 秒内无法移动和攻击，速度清零，不受重力。\textbf{定身与隐身不会同时存在}——若对手正在隐身则解除其隐身。定身期间可破除铜头铁臂。
  \item \textbf{解除条件}：对定身目标造成一定量伤害后定身破碎，目标被击退并进入硬直。破碎时施法者若有“破而后立”词条则冷却时间减少 5 秒。
  \item \textbf{冷却时间}：40 秒（从释放时计算）。
  \item \textbf{定身增伤}：被定身的目标受到攻击时触发额外伤害乘区。
\end{itemize}

\subsubsection{隐身术（STEALTH）}

\begin{itemize}
  \item \textbf{释放条件}：自身不在受击硬直中，且未被定身。仅限人机/无尽模式（双人模式不可选）。
  \item \textbf{效果}：角色变半透明，AI 失去对其视野（AI 将前往玩家最后已知位置巡逻）。持续 15 秒。期间可正常移动、攻击、拾取物品。
  \item \textbf{解除条件}：被定身时强制解除隐身，但按剩余时间比例返还冷却时间。若拥有“破影一击”词条，隐身时长减半，首次攻击伤害随隐身已过时间线性增长（1.0$\times$ 至 2.0$\times$），攻击后立即解除隐身。
  \item \textbf{冷却时间}：45 秒（从隐身开始计算）。
\end{itemize}

\subsubsection{铜头铁臂（IRON\_BODY）}

\begin{itemize}
  \item \textbf{效果}：激活后，近战攻击命中自身时——反弹此次伤害，攻击者被硬直 1 秒。如果攻击是远程（实心球/子弹），则将其以一定速度反弹回去且 parentId 变为防守方。
  \item \textbf{弱点}：被定身时铜头铁臂直接解除。
  \item \textbf{词条加成}：“荆棘”在反弹时对攻击者额外造成其攻击伤害的 50\%；“越战越勇”在反弹成功后减 CD 5 秒；“硬化”激活期间移速 +30\%。
\end{itemize}

\subsection{伤害计算公式}

攻击命中后的伤害结算遵循以下流水线（各乘区独立）：

\begin{enumerate}
  \item \textbf{基础攻击力} $\mathit{BaseAtk}$：读取攻击者的基础攻击力，叠加通用伤害加成（\texttt{damageBonusMultiplier}）、两败俱伤（1.5$\times$）、破影一击（1.0$\times$ 至 2.0$\times$ 线性增长）等乘区修正。
  \item \textbf{武器倍率}：乘以武器对应倍率——近战拳击 $\times 5$，小刀 $\times 15$；远程步枪子弹 $\times 30$，狙击子弹 $\times 60$；实心球 $\times 1$。
  \item \textbf{武器专精}：在武器倍率后继续叠加对应武器的专精加成（如拳击 +80\%、小刀 +40\% 等词条）。
  \item \textbf{实心球动能计算}（仅实心球）：不直接使用武器倍率，而根据发射初速和球的动能公式通过 sigmoid 映射得到伤害值，且单次伤害上限不超过目标最大生命值的 50\%。
  \item \textbf{定身受击增伤}：若防守方处于定身状态，触发 \texttt{freezeDmgMultiplier} 额外乘区。
  \item \textbf{护甲与受击减伤}：最后再进入护甲减伤 / 受击减伤阶段，统一写成 $\mathit{damage}$ 的后置衰减项。
\end{enumerate}

\textbf{核心规则}：所有词条均按独立乘区结算，词条等级叠加视为相同乘区内的线性累加（如两个“伤害+25\%”词条使 \texttt{damageBonusMultiplier} 从 1.0 变为 1.5，而非 $1.25 \times 1.25$）。

\subsection{词条系统}

词条（Modifier）是角色的被动增益，通过拾取地面上的词条卡片获得。人机/双人模式中，词条仅通过掉落物获取；无尽模式中，每轮结束自动弹出三选一词条面板。

词条分为两大类：\textbf{通用词条}（13 种，所有法术均可获取）和\textbf{法术专属词条}（每种法术 3-4 种，仅持有该法术时进入词条池）。完整列表如下：

\begin{table}[H]
\centering
\begin{tabular}{p{3cm}p{1.8cm}p{2.5cm}p{4.5cm}}
\toprule
\textbf{词条名称} & \textbf{类型} & \textbf{效果} & \textbf{备注} \\
\midrule
急转轮回 & 通用 & 法术冷却 -5秒/级 & 可叠加 \\
厚积薄发 & 通用 & 自身血量上限 +20\%/级 & 基于初始血量 \\
釜底抽薪 & 通用 & 对手血量上限 -20\%/级 & 施加到对手 \\
风驰电掣 & 通用 & 自身移速 +20\%/级 & — \\
缚风捉影 & 通用 & 对手移速 -15\%/级 & 施加到对手 \\
铁拳无敌 & 通用 & 拳击伤害 +80\%/级 & 可叠加 \\
刀光剑影 & 通用 & 小刀伤害 +40\%/级 & 可叠加 \\
石破天惊 & 通用 & 实心球伤害 +20\%/级 & 可叠加 \\
弹无虚发 & 通用 & 枪械伤害 +30\%/级 & 步枪/狙击共用 \\
锋芒毕露 & 通用 & 通用伤害加成 +25\%/级 & 武器倍率前乘区 \\
金刚不坏 & 通用 & 伤害减免 +20\%/级 & 上限 60\% \\
两败俱伤 & 通用 & 造成与受到伤害各 +50\% & \textbf{唯一} \\
百宝囊 & 通用 & 额外获得 1 武器槽位 & 可叠加 \\
\bottomrule
\end{tabular}
\caption{13 种通用词条。标注“唯一”的词条仅可获得一次（不可叠加等级）。未标注的为可叠加词条。}
\end{table}

法术专属词条按法术分类——定身法有“长缚”“缚中伤”“破而后立”等；隐身法有“长隐”“影疾”“潜息”“破影一击”等；铜头铁臂有“铜骨”“荆棘”“越战越勇”“硬化”等（完整列表见源码 \texttt{modifierdata.h}）。

\subsection{掉落物与拾取}

游戏进行中，地面会定时刷新或在击杀敌人后掉落物品：

\begin{itemize}
  \item \textbf{武器}：小刀、实心球、步枪、狙击枪——走近自动拾取，替换或补充对应槽位。
  \item \textbf{词条卡片}：走近自动拾取，调用 \texttt{applyModifier()} 立即生效。
  \item \textbf{血瓶/药包}：立即回复一定生命值、获得生命恢复效果或者临时增益。
\end{itemize}

所有掉落物有 300 帧（约 5 秒）存活时间，前 180 帧正常显示，最后 120 帧闪烁提示即将消失。

\subsection{AI 对手}

人机模式和无尽模式中的 P2 由 GameAI 控制。AI 具有两种随机人格（出生时随机分配）——“背水一战”（低血量时仍倾向进攻）和“游走机动”（优先搜寻物资）——使得每局 AI 行为不可预测。AI 能自主判断何时追击、何时找武器、何时施法、何时撤退，具体决策机制见技术四。

\subsection{对局结算}

单局模式中一方阵亡即结束，显示胜负画面，玩家按任意键返回主菜单。BO 模式下多局累计比分，先达到目标胜场数者赢得整场。无尽模式中玩家阵亡后显示结算面板——包含总伤害输出、总击杀数、生存时间和历史最佳记录——可选择“再来一局”复用配置或“返回菜单”。

\subsection{内容规模盘点}

在进入技术分析前，盘点这个“故意设计得复杂”的游戏到底有多少内容，以便理解六项技术在项目中的必要性：

\begin{itemize}
  \item 3 种游戏模式 $\times$ 2 种场景 = 6 种对局配置
  \item 5 种武器 $\times$ 各自的弹药、倍率、弹道行为
  \item 3 种法术（定身/隐身/铜头铁臂）$\times$ 各异的激活/冷却/解除机制
  \item 25+ 种词条（13 通用 + 每种法术 3-4 种专属）$\times$ 独立乘区叠加规则
  \item 8 个 AI 状态 + 2 种人格 + 特殊分支
  \item 11 个软件模块，40+ 源文件，跨平台构建
\end{itemize}

如果这些内容用手写 if-else 和全局变量来管理，代码早已不可维护。接下来的六章将逐一展示本项目用什么技术方案来应对这些复杂度。
...
这意味着：\textbf{CombatManager 不需要知道冲过来的是球还是子弹}。它只需调用虚函数接口。未来的新武器类型只需新建一个 Entity 子类，战斗管理器不用改一行代码。
...
项目中\textbf{没有}为不同武器类型创建子类（如 \texttt{class Punch : public Weapon}）。武器类型使用 \texttt{enum class WeaponType \{ punch, knife, ball, rifle, sniper \}} 表示，战斗计算中通过 \texttt{switch} 分支处理不同武器的倍率和行为。
...
\centering
\begin{tikzpicture}[
  class/.style={rectangle, draw, rounded corners, align=center,
                minimum width=2.2cm, minimum height=0.8cm, font=\small},
...
% ═══ 左-1（顶）：Entity 的三个子类 ═══
\node[class] (ball)                          {Ball};
\node[class, right=of ball]  (bullet)        {Bullet};
\node[class, right=of bullet] (drop)         {DropItem};

% ═══ 左中-2：Entity 基类（子类下方中央） ═══
\node[class, below=2.0cm of bullet] (entity) {Entity};

% ═══ 右-1（顶）：GameAI ═══
\node[class, right=3.0cm of drop] (gameai)   {GameAI};

% ═══ 右-2（中）：Player ═══
\node[class, below=2.0cm of gameai] (player) {Player};

% ═══ 中-2：CombatManager（对齐 DropItem） ═══
\node[class, below=2.0cm of drop] (combat) {CombatManager};

% ═══ 左中-3：Map ═══
\node[class, below=2.0cm of entity] (map)    {Map};

% ═══ 右-3：PlayerController ═══
\node[class, below=2.0cm of player] (pctrl)  {PlayerController};

% ── 继承：三个子类 → Entity（△在 Entity） ──
\draw[inh] (ball)   -- (entity);
\draw[inh] (bullet) -- (entity);
\draw[inh] (drop)   -- (entity);

% ── 聚合：GameAI / PlayerController → Player（◇在持有者） ──
\draw[agg] (gameai) -- (player);
\draw[agg] (pctrl)  -- (player);

% ── 关联：Map → Entity / PlayerController ──
\draw[dep] (map) -- (entity);
\draw[dep] (map) -- (pctrl);

% ── 关联：CombatManager → Entity（同行直线）/ PlayerController ──
\draw[dep] (combat) -- (entity);
\draw[dep] (combat) to[out=340, in=130] (pctrl);
\end{tikzpicture}
...
解决上述问题的核心思路是\textbf{观察者模式}：事件源不直接调用消费者，而是“广播”一个信号出去，由消费者自己决定是否订阅。这种模式在不同平台有不同的实现——Qt 有 signal/slot，Boost 有 \texttt{signals2}，C\# 有 delegate/event，甚至 C 语言也能通过函数指针回调列表手动实现。无论哪种实现，核心都是\textbf{把“谁发出事件”和“谁来响应事件”解耦}。
...
这样一来，\textbf{PlayerController 和实体创建逻辑是完全隔离的。}PlayerController 只负责”报告攻击请求”，如果将来需要给攻击添加屏幕震动或手柄振动，直接在 Widget 的 \texttt{onPlayerRequest} 槽函数中添加逻辑即可，PlayerController 不需要改一行代码。
...
项目并\textbf{没有}把所有模块间通信都交给信号槽。CombatManager 的战斗结算、SoundManager 的音效播放、Map 的碰撞查询，都通过直接函数调用完成。原因在于：
...
这个区分的依据是：\textbf{信号槽适用于“一对多”和“未来可能有新消费者”的场景，而直接调用适用于“一对一”和“时机固定”的场景。}把信号槽用在该用的地方，而不是不加区分地全部走信号槽。
...
\hline
\textbf{Qt 信号槽（当前方案）} & 观察者模式的语言级支持；类型安全（编译期检查信号与槽签名匹配）；支持动态连接/断开；Qt 框架内置，无需额外库 & 依赖 Qt 的 moc 预编译器和 Q\_OBJECT 宏；信号槽调用有轻微间接开销（对 60FPS 游戏可忽略）。本项目已使用 Qt 框架，无需额外引入依赖 \\
\bottomrule
\end{longtable}
...
\begin{itemize}
  \item \textbf{57 种词条类型}（13 种通用 + 44 种法术专属），全部在 \texttt{ModifierType} 枚举和工厂方法中定义，其中已实现法术（定身/隐身/铜头铁臂）的约 25 种可在游戏中获取
  \item \textbf{3 种法术}（定身、隐身、铜头铁臂），各有不同的激活条件、持续时间和交互规则
...
如果把这些全部硬编码进战斗代码，\texttt{getAttackDamage()} 会变成什么样子？每加一个词条就要加一个 if 分支，每加一个法术就要在伤害计算中插入一段特殊逻辑。57 种词条 $\times$ 5 种武器 $\times$ 多种法术互动的排列组合将产生数百个需要维护的条件分支。

更关键的是可修改性问题：如果要调整"把定身增伤从 20\% 改成 25\%"，只需改 \texttt{modifierdata.h} 中工厂方法里一个数值。如果全部硬编码，需要找到伤害计算函数中某一行埋在 500 行代码里的常量，还不敢保证改的是对的。
...
注意：\texttt{getAttackDamage()} 不知道“伤害+20\%”是哪来的——它只看到 \texttt{damageBonusMultiplier} 字段当前值是 1.25（吃了一个词条），它无法也不需区分这是哪个词条类型贡献的。这就是数据驱动的要点：\textbf{战斗逻辑对数据来源保持无知}。
...
\begin{quote}
\small 注释：tick() 中直接写了递减逻辑，未抽象为虚函数。原因是所有法术计时器共享完全相同的行为（递减 → 触零 → 关闭标志），抽象为虚函数只会增加代码跳转次数而不减少任何重复。这是数据驱动设计中的一个务实判断：\textbf{当数据的“结构”一样时，不值得为结构相同的类型引入多态}。
\end{quote}
...
\midrule
\endhead
词条池生成 & 定身法玩家进入无尽模式 & \texttt{poolForPlayer("freeze")} 返回 13 通用 + 3 法术专属，不包含其他法术的专属词条 & 通过 \\
\hline
...
% Three branches under state
\node[box, below left=0.6cm and 0.8cm of state] (attack)  {ATTACK \\ 近战 / 远程};
\node[box, below=of state]                       (follow)  {FOLLOW \\ 寻路追击};
\node[box, below right=0.6cm and 0.8cm of state](seek)    {SEEK\_WEAPON \\ / ARMOR / SUPPLY};

% recover under follow (not under state)
\node[box, below=1.0cm of follow] (recover) {卡住脱困 \\ / 施法后追击};

% Forward flow: south → north
\draw[arr] (special.south) -- (sense.north);
\draw[arr] (sense.south)  -- (state.north);
\draw[arr] (state.south)  -- (attack.north);
\draw[arr] (state.south)  -- (follow.north);
\draw[arr] (state.south)  -- (seek.north);
\draw[arr] (follow.south) -- (recover.north);

% Feedback edges: return to sense via side anchors
\draw[arr] (attack.west)  to[out=180, in=200] (sense.west);
\draw[arr] (follow.west)  to[out=180, in=215] (sense.west);
\draw[arr] (seek.east)    to[out=0,   in=340] (sense.east);
\draw[arr] (recover.east) to[out=0,   in=310] (sense.east);
\end{tikzpicture}
...
游戏主循环的时序设计解决的是一个\textbf{信息流编排问题}：在每 16ms 的时间窗口内，如何安排子系统执行顺序使得每步计算都基于“最新且一致”的信息。本项目的方案选择了最简单可维护的路径——单线程、固定顺序、可变时间步长——在 60FPS 要求下稳定运行，且调试时卡在某一步可以精确知道哪些数据已经更新、哪些还没变。
...
\hline
\textbf{qmake（Qt 原生构建工具）} & Qt 自带，默认配置简单 & qmake 的功能集远少于 CMake；跨平台能力弱（实际是一个增强版 Makefile 生成器）；Qt 官方已明确 qmake 进入维护模式，推荐 CMake \\
\hline
...
\begin{itemize}
  \item \textbf{操作系统}：Windows 10/11（主力开发与测试环境）。macOS/Linux 上理论可编译（CMakeLists.txt 已配置跨平台），但未做完整测试。
  \item \textbf{编译器}：MinGW GCC 11.2.0（Windows）。macOS 上使用 Apple Clang（Xcode Command Line Tools 自带），Linux 上使用 GCC/Clang。
  \item \textbf{Qt 版本}：Qt 6.5.3 LTS。兼容 Qt 6.2+ 任意版本。
  \item \textbf{构建系统}：CMake 3.16+，Ninja（推荐）/ Make
  \item \textbf{C++ 标准}：C++17
\end{itemize}

\subsection{项目文件清单}

提交包中与编译和运行相关的核心文件（共 40+ 源文件）：

\begin{verbatim}
CMakeLists.txt                  # CMake 构建配置（全文 90 行）
Resources/game_character.qrc    # Qt 资源文件（图片、音效）
main.cpp                        # 程序入口，解析命令行参数
widget.h / widget.cpp           # 游戏主界面 + 60FPS 帧循环
player.h / player.cpp           # 角色实体（属性、武器、词条、法术）
playercontroller.h / .cpp       # 输入意图转换与动作控制
entity.h / entity.cpp           # 实体基类（飞物/掉落物共有接口）
ball.h / ball.cpp               # 实心球（抛物线弹道）
bullet.h / bullet.cpp           # 子弹（直线弹道，步枪/狙击两种）
dropitem.h / dropitem.cpp       # 掉落物（武器/词条卡/血瓶）
combatmanager.h / .cpp          # 战斗管理器（碰撞→伤害→状态结算）
gameai.h / gameai.cpp           # AI 系统（FSM + 人格 + 寻路）
map.h / map.cpp                 # 地图碰撞体 + 平台间 BFS 寻路
animation.h / animation.cpp     # 精灵动画（帧序列加载/播放）
soundmanager.h / .cpp           # 音效管理（单例，预加载）
gamesession.h                   # 游戏会话配置结构体
spellstate.h                    # 法术运行时状态
modifierdata.h                  # 词条数据定义（header-only）
modifieroverlay.h / .cpp        # 无尽模式词条三选一弹窗
mainwindow.h / mainwindow.cpp   # 主窗口 + 页面导航（菜单/设置/游戏）
setupwidget.h / setupwidget.cpp # 开局设置页（模式/参数/法术选择）
endlessresultwidget.h / .cpp    # 无尽模式结算面板 + 排行榜
endlessrecordmanager.h / .cpp   # 无尽记录统计
CollisionResult.h               # 碰撞结果结构体
\end{verbatim}

\subsection{编译步骤}

\textbf{前置条件}：系统已安装 Qt 6.x（含 Widgets、Multimedia、Core 模块）、CMake 3.16+、支持 C++17 的编译器。

\begin{enumerate}
  \item \textbf{打开终端}，进入项目根目录（\texttt{CMakeLists.txt} 所在目录）。
  \item \textbf{配置}：
    \begin{verbatim}
    cmake -B build -G Ninja -DCMAKE_BUILD_TYPE=Release
    \end{verbatim}
    若未安装 Ninja，可省略 \texttt{-G Ninja}，CMake 使用平台默认生成器（Windows 上 NMake 或 VS；macOS/Linux 上 Make）。
  \item \textbf{编译}：
    \begin{verbatim}
    cmake --build build --config Release
    \end{verbatim}
    编译过程约需 1-2 分钟。若出现 \texttt{Could not find Qt6} 错误，指定 Qt 安装路径：
    \begin{verbatim}
  ...
  \item \textbf{运行}：
    \begin{verbatim}
    # Windows
    build\shooting_game.exe

    # macOS
    open build/shooting_game.app

    # Linux
    ./build/shooting_game
    \end{verbatim}
  \item \textbf{资源说明}：所有图片和音效通过 \texttt{.qrc} 编译进可执行文件，无需外置资源目录。如运行时无音效，检查 Qt Multimedia 模块是否正确安装（Windows 上确保 \texttt{Qt6Multimedia.dll} 在 PATH 中）。
\end{enumerate}

\subsection{编译验证}

在以下环境中成功编译并运行，帧率稳定在约 60FPS：

\begin{itemize}
  \item Windows 11 + MinGW GCC 11.2.0 + Qt 6.5.3 + Ninja
\end{itemize}

\subsection{运行与操作说明}

\begin{itemize}
  \item \textbf{首次启动}：点击“开始”进入设置页，选择“人机对战”模式，保持默认参数，点击“开始对局”。
  \item \textbf{P1 键位}：W/A/S/D 移动，F 攻击，G/C 切换武器槽，Q 释放法术。
  \item \textbf{P2 键位}（双人模式）：I/J/K/L 移动，O 攻击，N 切换武器槽，U 释放法术。
  \item \textbf{结束对局}：一方阵亡后显示结算画面，按任意键返回主菜单。
  \item \textbf{演示视频}：完整的玩法演示见提交附件中的视频文件。
\end{itemize}
...
完成这个项目让我深刻理解了“游戏开发不仅是写代码，更是设计系统”的含义。每一行代码的背后，都有对可维护性、可扩展性和性能的权衡；每一个模块的设计，都需要考虑它与其他模块的交互和依赖。通过这次实践，我不仅巩固了 C++ 和 Qt 的基础知识，也学会了如何在有限的时间内做出合理的技术选择。在一个看似简单的横版格斗游戏背后，往往隐藏着一系列精心设计的技术方案，它们共同支撑了游戏的可玩性、可维护性和可扩展性。

\vspace{1em}
\noindent 本项目所有源码、资源文件和本文档的 LaTeX 源码均随提交一并提供，根据要求，编译完毕的可执行文件不被包含，需要助教/老师根据自己的环境自行编译。
