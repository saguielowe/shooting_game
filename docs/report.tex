...
\begin{enumerate}
  \item \textbf{C++与Qt框架下的面向对象游戏架构设计}：从UI层（Widgets）到游戏逻辑层、战斗系统层的分层设计，学习如何在C++17标准下利用Qt的信号槽机制实现松耦合模块通信。特别是在处理游戏频率更新（一般60FPS）与UI事件循环协调时的设计要点。
...
本游戏的设计也参考了经典的2D平台射击游戏（如《合金弹头》、《魂斗罗》）的平衡原则，法术借鉴了《黑神话 悟空》中的设计思路，强调玩家技能和决策的重要性，同时通过合理的数值设计和反馈机制让游戏既有挑战性又不失公平感。

\subsection{模块划分}

本项目的模块划分是渐进的，记录与统计、Game AI、SoundManager等模块都是在核心功能实现后才逐步添加的。

\begin{itemize}
  \item \textbf{PlayerController} 负责输入和动作，不负责战斗结算，避免控制逻辑和战斗逻辑互相污染。
  \item \textbf{Player} 只负责状态和数值，不负责输入和AI，避免数据结构被操作逻辑绑死。
  \item \textbf{CombatManager} 统一管理碰撞和伤害结算，避免每个角色各写一套伤害规则。
  \item \textbf{GameAI} 单独维护决策逻辑，方便替换不同策略而不影响角色本体，AI的实体也是一个player，保持数据结构一致。
...
这种划分的核心收益是降低耦合度。比如，如果之后要给 AI 加新状态，只需要修改 AI 模块和少量接口；如果要调伤害，只需要碰战斗模块和数值数据；如果要换地图，只需要改地图和资源，不必重写控制系统。不过，由于Player和PlayerController本身就是紧密相关的，二者的职责分离虽然有助于代码清晰，但也增加了状态同步的复杂度。比如，PlayerController需要频繁查询Player的状态来判断是否可以攻击、是否受击中等，同时Player的状态更新又需要通过PlayerController来驱动动画和位置更新。这种“数据只存一份”的要求势必导致两者之间的接口调用频繁，改动一处可能会影响多处，需要在设计时格外注意接口的稳定性和清晰度。
...
\begin{itemize}
  \item \texttt{int currentHp, maxHp}：实时血量与上限。
  \item \texttt{QVector<WeaponType> weaponSlots}：武器槽与当前武器索引。
  \item \texttt{SpellState spellState}：记录法术的激活与冷却状态。
  \item \texttt{ModifierSet modifiers}：词条集合（被动属性修饰器）。
  \item \texttt{PlayerState state}：动作/硬直/受击等运行时标记。
\end{itemize}

\textbf{主要方法}：

\begin{itemize}
  \item \texttt{void update(float dt)}：按帧更新动画、冷却、状态过渡。
  ...
  \item \texttt{QStringList getModifierSummary()}：用于 UI 显示的简要词条文本。
\end{itemize}
...
\begin{itemize}
  ...
  \item 寻路精度低：返回的是"下一个平台"而非精确路点，AI移动需要额外支持。
...
\begin{itemize}
  \item \textbf{掉落机制}：角色击杀敌人或到达时间阈值时，在该位置生成 \texttt{DropItem} 对象。掉落物包括武器、词条、血瓶等。
  \item \textbf{物品拾取}：玩家移近掉落物时自动拾取，更新武器或词条列表。
  \item \textbf{词条系统}：玩家可同时应用多个词条（如"伤害+20\%"、"伤害+15\%"等），词条修饰在战斗流程中被计算。
\end{itemize}

掉落物与实心球类似，都是 \texttt{Entity} 的子类，具有位置、碰撞箱、渲染方法等。掉落物的类型由 \texttt{DropItem::type} 区分，拾取时根据类型调用不同的处理逻辑（如武器替换、词条添加）。
...
当玩家拾取词条后，该词条被添加到 \texttt{Player::modifiers} 容器。战斗计算时，遍历 \texttt{modifiers} 并应用所有激活的词条。这样，新增词条只需定义新的数据条目，无需改动战斗计算代码。
...
原先考虑过高层意图（如“先发育”，“先攻击”），配合低层动作（如“向左移动”，“跳跃”）的分层状态机，但实际实现中发现，高层意图脱离实际，计算“发育分数”并不现实。因此，当前实现更侧重于基于行为+性格概率的状态机：
...
%重写此部分，不要写犄角旮旯的特殊分支，根据代码实际来讲述主要决策路径，完整的且远远不止follow状态。写determine next state%
