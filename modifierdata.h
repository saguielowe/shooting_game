...
inline QVector<ModifierData> ModifierData::allGeneric() {
    return {
        { ModifierType::SPELL_COOLDOWN_REDUCE, "急转轮回",    "法术冷却 -5秒" },
        { ModifierType::MAX_HP_UP,             "厚积薄发",    "自身血量上限 +20%" },
        { ModifierType::ENEMY_MAX_HP_DOWN,     "釜底抽薪",    "对手血量上限 -20%" },
        { ModifierType::MOVE_SPEED_UP,         "风驰电掣",    "自身移动速度 +20%" },
        { ModifierType::ENEMY_MOVE_SPEED_DOWN, "缚风捉影",    "对手移动速度 -15%" },
        { ModifierType::PUNCH_DAMAGE_UP,       "铁拳无敌",    "拳击伤害 +80%" },
        { ModifierType::KNIFE_DAMAGE_UP,       "刀光剑影",    "小刀伤害 +40%" },
        { ModifierType::BALL_DAMAGE_UP,        "石破天惊",    "实心球伤害 +20%" },
        { ModifierType::GUN_DAMAGE_UP,         "弹无虚发",    "枪械伤害 +30%" },
        { ModifierType::DAMAGE_BONUS_UP,       "锋芒毕露",    "通用伤害加成 +25%" },
        { ModifierType::DAMAGE_REDUCTION_UP,   "金刚不坏",    "伤害减免 +20%（上限60%）" },
        { ModifierType::DOUBLE_EDGE,           "两败俱伤",    "造成与受到的伤害都 +50%" },
        { ModifierType::EXTRA_WEAPON_SLOT,     "百宝囊",      "额外获得 1 个武器槽位" },
    };
}
...
inline QVector<ModifierData> ModifierData::forSpell(const QString& spellName) {
    if (spellName == "freeze") {
        return {
            { ModifierType::FREEZE_DURATION_UP,  "长缚",     "定身持续时间 +1.5秒" },
            { ModifierType::FREEZE_DAMAGE_UP,    "缚中伤",   "定身期间伤害 +20%" },
            { ModifierType::FREEZE_BREAK_CDR,    "破而后立", "定身破碎后冷却 -5秒（唯一）" },
        };
    }
    if (spellName == "stealth") {
        return {
                { ModifierType::STEALTH_DURATION_UP, "长隐",   "隐身持续时间 +2秒" },
                { ModifierType::STEALTH_SPEED_UP,    "影疾",   "隐身时移动速度 +20%" },
                { ModifierType::STEALTH_REGEN,       "潜息",   "隐身时每秒回血 +2点" },
                { ModifierType::STEALTH_FIRST_HIT,   "破影一击",
                 "隐身时长减半，首击伤害随隐身时长提升，并解除隐身（唯一）" },
                };
    }
    if (spellName == "barrier") {
        return {
            { ModifierType::BARRIER_DURATION_UP,   "久安",   "屏障持续时间 +5秒" },
    ...
    if (spellName == "clone") {
        return {
            { ModifierType::CLONE_CAN_CAST_SPELL,  "分身施法", "克隆体可使用法术 1 次（唯一）" },
            { ModifierType::CLONE_DAMAGE_UP,       "分身锋利", "克隆体伤害 +20%" },
            { ModifierType::CLONE_DURATION_UP,     "长影",     "克隆体持续时间 +5秒" },
            { ModifierType::CLONE_DAMAGE_EXTEND,   "以伤续命", "按造成伤害延长持续时间（唯一）" },
        };
    }
    if (spellName == "iron_body") {
        return {
            { ModifierType::IRON_BODY_DURATION_UP, "铜骨",   "铜头铁臂持续时间 +1秒" },
            { ModifierType::IRON_BODY_REFLECT_CDR, "越战越勇", "成功弹反一次，法术冷却 -5秒（每次激活仅生效一次，唯一）" },
            { ModifierType::IRON_BODY_THORNS,      "荆棘",   "近战弹反时，攻击者额外受到其攻击伤害50%（唯一）" },
